...
window.showNotification = function(type, message) {
    ...
    if (!$container.length) {
        jQuery('body').append('<div id="psycho-notification-container" style="position: fixed; top: 20px; right: 20px; z-index: 999999; max-width: 400px;"></div>');
...
(function($) {
    ...
    const colorSchemes = {
        'template-standard': {
            ...
            colors: ['#2F6D67', '#6FA89F', '#1A1F23', '#0D47A1', '#8C6E00', '#C0392B', '#FAFAF8', '#F2F5F3', '#5B6366', '#D9E3DF']
        },
        'sage-green': {
            name: '🌿 Salbeigrün',
            colors: ['#8FA88F', '#B8C9B8', '#2A2F2A', '#5B8CA8', '#A89D5B', '#A85B5B', '#F7F9F7', '#EFF3EF', '#6B716B', '#D4DDD4']
        },
        'soft-lavender': {
            name: '💜 Sanfte Lavendeltöne',
            colors: ['#7B88B5', '#A8B3D4', '#2E2E3A', '#6A9AB0', '#B5A67C', '#A86B6B', '#F9F9FC', '#F0F0F5', '#6B6B7D', '#D8D8E8']
        },
        'warm-rose': {
            name: '🌸 Warme Rosétöne',
            colors: ['#D4877B', '#E8AFA6', '#3A2E2E', '#7FA3B8', '#D4A87B', '#B86B6B', '#FCF8F7', '#F5EFED', '#7D6E6B', '#E8DDD8']
        },
        'warm-blue-apricot': {
            name: '🌊 Blaugrau/Apricot',
            colors: ['#7A93A8', '#A8BDC9', '#232A2E', '#5B7FA8', '#D4A03D', '#C64B42', '#FAFBFC', '#EEF3F5', '#656B70', '#D6E1E7']
        },
        'chocolate-mokka': {
            name: '☕ Schokobraun',
            colors: ['#6B5447', '#9B8577', '#2F2926', '#5B7FA8', '#A88D47', '#A84D47', '#FAF8F7', '#F0EBE8', '#6B6560', '#DDD5D0']
        },
        'clear-blue': {
            name: '💙 Klares Mittelblau',
            colors: ['#5B8CB8', '#8FB3D4', '#232A2F', '#4A7BA7', '#B8A047', '#B84D4D', '#F7FAFC', '#EDF3F7', '#5F666B', '#D4E1EA']
        },
        'deep-terracotta': {
            name: '🍂 Tiefes Terracotta',
            colors: ['#A85840', '#D4936B', '#2F2623', '#5B84A8', '#B89447', '#B84747', '#FCF8F6', '#F5EEEA', '#6F6560', '#E8DDD4']
        },
        'soft-mauve': {
            name: '🪻 Weiches Mauve',
            colors: ['#A8859B', '#C9B3BE', '#2A2628', '#6B8AA8', '#C9A350', '#B84D4D', '#FDFBFC', '#F5F0F3', '#6F696D', '#E3D8DE']
        }
    };
    ...
    $(document).ready(function() {
        ...
        const noAutoJump = sessionStorage.getItem('psycho_wizard_no_autojump') === 'true';
    ...
    function loadWizardStatus(autoJump = true) {
        $.ajax({
            ...
            success: function(response) {
                if (response.success) {
                    ...
                    if (status.hello_theme_active) {
                        $('#helloTheme').removeClass('status-pending').addClass('status-installed').text(psychoWizard.i18n.installed);
                        autoCompletedSteps.push(2);
                    }

                    // Step 3: Elementor
                    if (status.elementor_active) {
                        $('#elementor').removeClass('status-pending').addClass('status-installed').text(psychoWizard.i18n.installed);
                    ...
                    } else if (currentStep === 5 || (status.wizard_status && status.wizard_status.current_step === 5)) {
                    ...
                    if (status.wizard_status && status.wizard_status.styling_plugin_installed) {
                        $('#stylingPlugin').removeClass('status-pending').addClass('status-installed').text(psychoWizard.i18n.installed);
                    ...
                    if (status.wizard_status && status.wizard_status.privacy_page_published) {
                    ...
                    if (status.wizard_status && status.wizard_status.completed_steps) {
                        ...
                        allCompletedSteps = [...new Set([...autoCompletedSteps, ...status.wizard_status.completed_steps])];
                    }
                    ...
                    if (allCompletedSteps.length > 0) {
                        ...
                        if (allCompletedSteps.length >= totalSteps - 1) {  // -1 weil Step 1 nicht gezählt wird
                    ...
                    console.log('continueSetupCard exists:', $('#continueSetupCard').length);

                    if (nextIncompleteStep > 1 && $('#continueSetupCard').length) {
                        console.log('Zeige Fortsetzen-Kachel für Step:', nextIncompleteStep, stepNames[nextIncompleteStep]);
                        $('#nextStepName').text(stepNames[nextIncompleteStep]);
                        $('#continueSetupCard').fadeIn();

                        // Button Handler zum Springen
                        $('#jumpToNextStep').off('click').on('click', function() {
                            console.log('Springe zu Step:', nextIncompleteStep);
                            currentStep = nextIncompleteStep;
                            updateSteps();
                            updateProgress();
                            window.updateButtons();
                            window.showNotification('success', 'Weiter bei Schritt ' + nextIncompleteStep + ': ' + stepNames[nextIncompleteStep]);
                        });
                    } else {
                        console.log('Kachel wird NICHT angezeigt. nextIncompleteStep:', nextIncompleteStep, 'Card exists:', $('#continueSetupCard').length);
                    }
                    ...
                    if (autoJump && nextIncompleteStep > 1 && currentStep !== 1) {
                        ...
                        window.showNotification('success', 'Fortschritt wiederhergestellt! Weiter bei Schritt ' + nextIncompleteStep);
            ...
            error: function() {
                window.showNotification('error', 'Fehler beim Laden des Status. Starte von vorne.');
            }
        });
    }
    ...
    function setupEventHandlers() {
        ...
        $(document).on('click', '.step', function() {
            ...
            if (targetStep && targetStep !== currentStep && targetStep >= 1 && targetStep <= totalSteps) {
                console.log('Springe von Step', currentStep, 'zu Step', targetStep);
                currentStep = targetStep;
                updateSteps();
                updateProgress();
                window.updateButtons();
                window.showNotification('info', 'Zu Schritt ' + targetStep + ' gesprungen');
            }
        });
    ...
    function uploadElementorPro(file) {
        ...
        window.showNotification('info', 'Elementor Pro wird hochgeladen... Dies kann einen Moment dauern.');
        ...
        $.ajax({
            ...
            success: function(response) {
                if (response.success) {
                    ...
                    window.showNotification('success', response.data.message + ' - Aktiviere jetzt deine Lizenz.');
                } else {
                    window.showNotification('error', response.data.message);
                }
            },
            error: function() {
                window.showNotification('error', 'Upload fehlgeschlagen. Bitte versuche es erneut.');
            }
        });
    }
    ...
    window.checkTemplateKitImport = function() {
        ...
        $.ajax({
            ...
            success: function(response) {
                ...
                $btn.prop('disabled', false).text(psychoWizard.i18n.checkImportStatusBtn);
                ...
                if (response.success && response.data.has_templates) {
                    ...
                    $statusText.html(psychoWizard.i18n.templateKitImported + '<br>' + foundText);
                    ...
                    $statusText.html('⚠️ <strong>No templates found yet</strong><br>' +
                                   'Please import your kit via Elementor first, then check again.<br>' +
                                   (response.data.message || 'No templates or pages found. Please import your kit via Elementor.'));
                    window.showNotification('error', 'No templates imported yet. Please perform the import in Elementor first.');
                }
            },
            error: function(xhr, status, error) {
                console.error('Template import check error:', xhr.responseText);
                $btn.prop('disabled', false).text(psychoWizard.i18n.checkImportStatusBtn);
                window.showNotification('error', 'Error checking import status: ' + error);
            }
        });
    };
    ...
    window.checkElementorProLicense = function() {
        ...
        $.ajax({
            ...
            success: function(response) {
                ...
                $btn.prop('disabled', false).text(psychoWizard.i18n.checkLicenseStatusBtn);
                ...
                if (response.success && response.data.is_active) {
                    ...
                    uploadedFiles.elementorProActivated = true;
                    $statusDisplay.css('background', '#d1fae5').css('border', '2px solid #10b981').show();
                    $statusText.html('✅ <strong>License active!</strong> You can now proceed.');
                    window.updateButtons();

                    if (wasAlreadyActive) {
                        window.showNotification('success', '✓ Status checked - License is still active!');
                    } else {
                        window.showNotification('success', 'Elementor Pro license successfully activated!');
                    }
                } else {
                    // License not yet active
                    $statusDisplay.css('background', '#fee2e2').css('border', '2px solid #ef4444').show();
                    $statusText.html('❌ <strong>License not yet active</strong><br>Please activate Elementor Pro via the link above and check again.');
                    window.showNotification('error', 'License not yet activated. Please activate it first.');
                }
            },
            error: function(xhr, status, error) {
                console.error('License check error:', xhr.responseText); // Debug
                $btn.prop('disabled', false).text(psychoWizard.i18n.checkLicenseStatusBtn);
                window.showNotification('error', 'Error checking license status: ' + error);
            }
        });
    };
    ...
    function uploadTemplateKit(file) {
        // Prüfe Dateityp
        if (!file.name.endsWith('.zip')) {
            window.showNotification('error', 'Bitte lade eine ZIP-Datei hoch (Elementor Website Kit)');
            return;
        }
        ...
        window.showNotification('info', 'Template Kit wird hochgeladen und importiert... Dies kann 1-3 Minuten dauern.');
        ...
        $.ajax({
            ...
            error: function(xhr, status, error) {
                ...
                if (status === 'timeout') {
                    window.showNotification('error', 'Upload-Timeout. Die Datei ist zu groß oder der Server zu langsam.');
                } else {
        ...
        $.ajax({
            ...
            success: function(response) {
                if (response.success) {
                    ...
                    window.showNotification('info', '🔄 Seite wird neu geladen um den Custom Post Type anzuzeigen...');
    ...
    function activateLicense() {
        ...
        $.ajax({
            ...
            error: function(xhr, status, error) {
                $nextBtn.prop('disabled', false).text(originalText);
                window.showNotification('error', 'Fehler bei der Lizenzaktivierung: ' + error);
            }
        });
    }
        ...
        $.ajax({
            ...
            success: function(response) {
                ...
                if (response.success) {
                    ...
                    $(`.color-scheme[data-scheme="${selectedScheme}"]`).addClass('active');

                    // Entferne "Eigenes Farbschema aktiv" Info-Box falls vorhanden
                    $('#colorSchemesContainer > .info-box').remove();

                    window.showNotification('success', '✅ Farbschema erfolgreich angewendet!');
                    ...
                    $(`.color-scheme[data-scheme="${selectedScheme}"]`).removeClass('active');
                    window.showNotification('error', response.data.message || 'Fehler beim Anwenden der Farben');
                }
            },
            error: function(xhr, status, error) {
                isApplyingColors = false;
                ...
                $(`.color-scheme[data-scheme="${selectedScheme}"]`).removeClass('active');
                window.showNotification('error', 'Fehler beim Anwenden der Farben: ' + error);
    ...
    function applyColors() {
        ...
        window.showNotification('info', '🎨 Farbschema wird angewendet, dies kann einen Moment dauern...');
        ...
        $.ajax({
            ...
            success: function(response) {
                ...
                if (response.success) {
                    ...
                    window.showNotification('success', '✓ Alle 17 Farben erfolgreich angewendet!');
    ...
    function completeWizard() {
        $.ajax({
            ...
            success: function(response) {
                if (response.success) {
                    window.showNotification('success', '✅ Setup erfolgreich abgeschlossen!');
    ...
    window.nextStep = function() {
        ...
        if (currentStep === 4 && uploadedFiles.elementorPro && $('#proLicense').val()) {
            activateLicense();
            return;
        }
    ...
    function setupStepUploads() {
        // Step 6: ACF JSON
        if (currentStep === 6 && $('#acfUpload').length && !$('#acfUpload').data('upload-ready')) {
        ...
        // Step 7: Demo Data
        if (currentStep === 7 && $('#demoUpload').length && !$('#demoUpload').data('upload-ready')) {
        ...
        // Step 8: Styling Plugin
        if (currentStep === 8 && $('#stylingPluginUpload').length && !$('#stylingPluginUpload').data('upload-ready')) {
            ...
            if (!$('#importDemoCheckbox').data('handler-registered')) {
                ...
                $('#importDemoCheckbox').on('change', function() {
                    if (this.checked) {
                        ...
                        $('#skipDemoBtn').hide();
                        if ($('#demoDataUpload').length && !$('#demoDataUpload').data('upload-ready')) {
                            console.log('Setting up Demo Data upload area');
        ...
        // Step 10: Logo Upload
        if (currentStep === 10 && $('#logoUploadArea').length && !$('#logoUploadArea').data('upload-ready')) {
        ...
        if (currentStep === 12) {
            if ($('#headingFontUpload').length && !$('#headingFontUpload').data('upload-ready')) {
            ...
            if ($('#bodyFontUpload').length && !$('#bodyFontUpload').data('upload-ready')) {
            ...
            $.ajax({
                ...
                success: function(response) {
                    if (response.success && response.data.scheme) {
                        ...
                        $(`.color-scheme[data-scheme="${activeScheme}"]`).addClass('active');
                        selectedScheme = activeScheme;

                        window.updateButtons();
                    } else {
                        console.log('ℹ️ Kein gespeichertes Schema gefunden - versuche Fallback-Erkennung...');
                        ...
                        $.ajax({
                            ...
                            success: function(colorResponse) {
                                if (colorResponse.success && colorResponse.data.colors) {
                                    ...
                                    for (const [schemeKey, schemeData] of Object.entries(colorSchemes)) {
                                        ...
                                        for (let i = 0; i < 10; i++) {
                                            if (currentColors[i] && schemeColors[i]) {
                                                if (currentColors[i].toUpperCase() === schemeColors[i].toUpperCase()) {
                                                    matches++;
                                                }
                                            }
                                        }

                                        console.log(`Schema "${schemeKey}": ${matches}/10 Farben übereinstimmend`);
                                    ...
                                    if (matchedScheme) {
                                        console.log('✅ Schema durch Farbvergleich erkannt:', matchedScheme);
                                        $(`.color-scheme[data-scheme="${matchedScheme}"]`).addClass('active');
            ...
            $.ajax({
                ...
                success: function(response) {
                    if (response.success && response.data.scheme) {
                        ...
                        // Markiere das aktive Schema
                        $(`.typography-scheme[data-scheme="${activeScheme}"]`).addClass('active');
                    } else {
                ...
                error: function(xhr, status, error) {
                    console.error('Fehler beim Laden des aktiven Typography-Schemas:', error);
                    window.updateButtons();
                }
            });
        }
    ...
    window.updateButtons = function() {
        ...
        switch(currentStep) {
            ...
            case 4:
                ...
                canProceed = uploadedFiles.elementorPro && uploadedFiles.elementorProActivated;
                buttonText = uploadedFiles.elementorProActivated ? psychoWizard.i18n.nextBtn : psychoWizard.i18n.pleasActivateLicenseBtn;
                break;
            case 5:
                // Template Kit: Nur weiter wenn importiert
                canProceed = uploadedFiles.templateKit;
                buttonText = uploadedFiles.templateKit ? psychoWizard.i18n.nextBtn : psychoWizard.i18n.pleaseImportTemplateKitBtn;
                break;
            case 6:
    ...
    function previewScheme(scheme) {
        ...
        $('#previewBox').css('background', `linear-gradient(135deg, ${colors[0]} 0%, ${colors[1]} 100%)`);
    ...
    $(document).on('click', '.typography-scheme', function() {
        ...
        $.ajax({
            ...
            success: function(response) {
                if (response.success) {
                    showNotification('✅ ' + (response.data?.message || 'Schriftarten erfolgreich geändert!'), 'success');
                    ...
                    $('.typography-scheme[data-scheme="' + scheme + '"]').removeClass('active');
                }
            },
            error: function() {
                showNotification('❌ Verbindungsfehler beim Anwenden der Schriftarten', 'error');
                $('.typography-scheme[data-scheme="' + scheme + '"]').removeClass('active');
            }
        });
    });
